...
if __name__ == "__main__":
    ...
    if (run_state == run_states.NEW or set_data):
        ...
        state:SegmentState = SegmentGenerator.generate(GenerationOptions())[0];  # type: ignore
        x,y = state.task;
        path = [(x,y),(x+c.TILE_SIZE*2,y),(x+c.TILE_SIZE*4,y),(x+c.TILE_SIZE*6,y),(x+c.TILE_SIZE*8,y),(x+c.TILE_SIZE*10,y)];
        print(path[0]);
        print(state.task);
        state.task_path = path;
        tdManager.set_data([state]);
    if add_data:
